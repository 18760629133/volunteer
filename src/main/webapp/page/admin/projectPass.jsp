...
<div class="ok-body">
    ...
    <form class="layui-form layui-form-pane ok-form" lay-filter="filter">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="starttime" id="startTime" placeholder="请输入招募开始时间" autocomplete="off" class="layui-input" lay-verify="required">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">招募结束时间</label>
            <div class="layui-input-block">
                <input type="text" name="endtime" id="endTime" placeholder="招募结束时间" autocomplete="off" class="layui-input" lay-verify="required">
            </div>
        </div>
        <div class="layui-form-item">
...
<script>
    //初始化数据，从待审核项目数据列表中获取数据，传到审核项目子页面
    let initData;
    function initForm(data) {
        let jsonString = JSON.stringify(data);
        initData = JSON.parse(jsonString);
    }
    layui.use(["element","form", "okUtils", "okLayer","laydate"], function () {
        let form = layui.form;
        ...
        form.on("submit(addArticle)", function (data) {
            okUtils.ajax("${pageContext.request.contextPath}/project/passProject", "post", data.field, true).done(function (response) {
